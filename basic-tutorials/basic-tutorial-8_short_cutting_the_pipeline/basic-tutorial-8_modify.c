...
int main(int argc, char *argv[])
{
    ...
    pad_template = gst_element_get_pad_template(data.tee, "src_%u");
    if (!pad_template)
    {
    ...
    tee_audio_pad = gst_element_request_pad(data.tee, pad_template, NULL, NULL);
    if (!tee_audio_pad)
    {
    ...
    tee_video_pad = gst_element_request_pad(data.tee, pad_template, NULL, NULL);
    if (!tee_video_pad)
    {
    ...
    tee_app_pad = gst_element_request_pad(data.tee, pad_template, NULL, NULL);
    if (!tee_app_pad)
    {
        g_printerr("Failed to request audio pad from tee\n");
        gst_element_release_request_pad(data.tee, tee_audio_pad);
...
// gcc basic-tutorial-8_modify.c -o basic-tutorial-8_modify `pkg-config --cflags --libs gstreamer-1.0 gstreamer-audio-1.0`
